...
class CompoundProteinInteractionPrediction(nn.Module):
    def __init__(self, top_words, seq_len, o_dim):
        super(CompoundProteinInteractionPrediction, self).__init__()
        self.embedding_vecor_length = 64
        self.seq_len = seq_len
        self.top_words = top_words

        self.embedding = nn.Embedding(
            self.top_words, self.embedding_vecor_length)
        self.dropout1 = nn.Dropout(p=0.4)
        self.gru = nn.GRU(input_size=self.embedding_vecor_length,
                          hidden_size=100, batch_first=True, bidirectional=True)
        self.dense1 = nn.Linear(in_features=200, out_features=1)
        self.flatten = nn.Flatten()
        self.softmax = nn.Softmax(dim=-1)
        self.repeat = nn.Replicate(200, dim=2)
        self.permute = nn.Permute([0, 2, 1])
        self.multiply = nn.MultiheadAttention(embed_dim=200, num_heads=1)
        self.lambda_fn = lambda xin: torch.sum(xin, dim=1)
        self.dropout2 = nn.Dropout(p=0.5)
        self.dense2 = nn.Linear(in_features=200, out_features=o_dim)
        self.dense3 = nn.Linear(in_features=200, out_features=o_dim)
    ...
    def forward(self, x):
        ...
        att = self.repeat(att)
        att = self.permute(att)
        x, _ = self.multiply(x, x, x, key_padding_mask=None)
        x = torch.mul(x, att)
        x = self.lambda_fn(x)
        x = self.dropout2(x)
        y1 = F.softmax(self.dense2(x), dim=-1)
        y2 = torch.sigmoid(self.dense3(x))
        return y1, y2
...
if __name__ == "__main__":
    ...
    """Hyperparameters."""
    DATASET, radius, ngram, dim, layer_gnn, window, layer_cnn, layer_output, lr, lr_decay, decay_interval, weight_decay, iteration = 'celegans', 2, 4, 128, 3, 11, 3, 3, 1e-3, 0.5, 10, 1e-6, 31
    n_val = 5
